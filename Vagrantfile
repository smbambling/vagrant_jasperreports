...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  # Foreman Server
  config.vm.define "jasper".to_sym do |jasper|
    jasper.vm.box = BOX
    jasper.vm.hostname = "jasper" + "." + DOMAIN
    jasper.hostmanager.aliases = %w(jasper)
    jasper.vm.network :public_network, :adapter => 3 
    jasper.vm.network :private_network, :auto_network => true
    #set some machine-specific options  
    jasper.vm.provider "virtualbox" do |v|
      #change the amount of RAM 
      v.customize ["modifyvm", :id, "--memory", "1024"]
      v.customize ["modifyvm", :id, "--cpus", "2"]
    end
    #run bootstrap scripts
    jasper.vm.provision :shell, :path => "bootstrap_scripts/puppet_agent.sh"
    jasper.vm.provision :puppet, 
